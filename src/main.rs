...
use kube::{Api, CustomResource, ResourceExt, Client};
...
fn upsert_ready_condition(conditions: &mut Vec<Condition>, status: &str, reason: Option<&str>, message: Option<&str>) {
    let now = Utc::now();
